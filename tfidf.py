...
def create_training(TWC, D, training_udocIDs, tf_type, idf_type, lemmatize=True):
	...
	if not os.path.exists(fn):
		...
		pickle.dump(training_TFIDF, open( dest_fn+'.train.pkl', 'wb'), pickle.HIGHEST_PROTOCOL)
	# else:
...
def create_testing(TWC, D, testing_udocIDs, tf_type, idf_type, lemmatize=True):
	...
	if not os.path.exists(fn):
		...
		pickle.dump(testing_TFIDF, open( dest_fn+'.test.pkl', 'wb'), pickle.HIGHEST_PROTOCOL)
